...
export class ProductsService {
    ...
    findAll(): Promise<Product[]> {
        return this.productsRepository.find({
            relations: ['productSaleslocation'], //조인하여 가져오기
        });
    }
    ...
    async create({
        ...
        const { productSaleslocation, ...product } = createProductInput; /// ...product 안에는 productSaleslocation 이외에 값들이 복사되어 있다.
        ...
        const result2 = await this.productsRepository.save({
            ...
            productSaleslocatin: result, //result 통쨰로 넣기(앞에서 저장한 주소데이터 주소외래키값 포함)
